...
export function CreateAccountPage() {
    ...
    async function handleAccountCreation(e: FormEvent<HTMLFormElement>) {
        e.preventDefault();
        const newUserDetailsToSend = {username, password, email};
        try {
            await createAccount(newUserDetailsToSend);
            navigate('/home')
        } catch (error) {
            console.error(error);
            ...
            <div className="createaccount-content">
                ...
                <form onSubmit={handleAccountCreation} className="createaccount-form">
                    ...
                    <Input
                        ...
                        onChange={setEmail}
                    />
                    <button type="submit" className="createaccount-button" disabled={!username || !password || !email}>Create account</button>
                </form>
                {!!accountCreationError && (
                    <p className="login-error-message">Oops! looks like something went wrong, please check your details.</p>
                )}
                <Link className="link-to-loginpage" to={`/`}>Return to log in page</Link>
            </div>
        </div>
    )
}